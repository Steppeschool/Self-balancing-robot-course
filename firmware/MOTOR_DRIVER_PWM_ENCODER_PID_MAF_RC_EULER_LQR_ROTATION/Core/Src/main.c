...
float temp_velocity;
uint8_t timer_flag;
int32_t duty_cyclech1, duty_cyclech2;
icm_20948_data icm_data;
imu_norm imu_norm_data;
euler_angles euler_angles_robot, euler_angles_old;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	if (htim == &htim8)
	{
		if(first_time1 == 0)
		{
			...
			{
				duty_cyclech1 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2) - RC_OFFSET;
			}
	...
	if (htim == &htim15)
	{
		if(first_time2 == 0)
		{
			...
			if(htim ->Channel == HAL_TIM_ACTIVE_CHANNEL_2)
			{
				duty_cyclech2 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2) - RC_OFFSET;
			}
		}
		else
		{
...
//   disable_motor((motor_inst*)&motor_b);
  ...
  while (1)
  {
	  ...
	  {
			timer_flag = 0;
			reference1 += (float32_t)duty_cyclech1 / 60000;
			reference2 += (float32_t)duty_cyclech2 / 160000;
			...
			//		  set_speed_open((motor_inst*)&motor_a, (duty_cyclech1 / 100));
			//		  set_speed_open((motor_inst*)&motor_b, (duty_cyclech2 / 100));
			icm_20948_read_data(&icm_data);
			sensor2imu(icm_data, &imu_norm_data);
			complementary_filter_euler(imu_norm_data,  &euler_angles_robot);
			apply_pid(&mot_rotation_pid, reference2 + motora_enc.position - motorb_enc.position);
			robot_state.angle = euler_angles_robot.pitch;
			robot_state.angle_vel = robot_state.angle_vel * 0.8 + 0.2 * (euler_angles_robot.pitch -
						  euler_angles_old.pitch) * SAMPLE_RATE;
			robot_state.position = reference1 + (motora_enc.position + motorb_enc.position) / 2 ;
			...
			if (euler_angles_robot.pitch < 1.0 && euler_angles_robot.pitch > -1.0 &&
					euler_angles_robot.roll < 0.9 && euler_angles_robot.roll > -0.9)
			{
			  apply_force((motor_inst*)&motor_a, force / 2 -
					  mot_rotation_pid.output / 2, motora_enc.velocity);
			  apply_force((motor_inst*)&motor_b, force / 2 +
					  mot_rotation_pid.output / 2, motorb_enc.velocity);
			}
			else
			{
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOC, ICM_SPI_CS_Pin|STSP_RST_Pin|PHA_Pin, GPIO_PIN_RESET);
